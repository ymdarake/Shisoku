...
export const generateProblem = (difficulty: Difficulty = 'normal'): Problem | null => {
  const operators = ['+', '-', '*', '/'];
  const MAX_TRIES = 5000;
  const { numberRange, targetRange } = DIFFICULTY_CONFIGS[difficulty];
          ...
          for (const op3 of operators) {
            ...
            for (const exp of expressions) {
              ...
              if (
                ...
                result >= targetRange[0] &&
                result <= targetRange[1]
              ) {
...
export const generateProblems = (count: number, difficulty: Difficulty = 'normal'): Problem[] => {
